...
.. glossary::
    ...
    DataWidget
        DataWidget is one of the main DreamFace components. It is a visual component which contains graphical controls linked to DataQueries to display information and data. DataWidgets are displayed in a composition on a screen to
        create the user interface of a DreamFace application. DataWidgets can be created visually with the Widget Editor or in the Jade Widget Editor where the Jade templating language for Express/Node.js is used. DreamFace has two
        Widget Editors for visually composing widgets, one for create widgets for web applications and one for creating widgets for Mobile applications.
    DFX
        DreamFace X-Platform (DFX) or just DreamFace, is a cloud application platform for developing web and mobile applications.
    Docker
